...
async function __goalLoad(){
  ...
  // Earned so far: closed files this year where she is either the primary agent or
  ...
  __txRows.forEach(function(t){
    ...
    if(t.gross_commission == null) return;
    var gross = Number(t.gross_commission) || 0;
    var share = Number(t.co_agent_share) || 0;
    var credit = isPrimary ? gross * (1 - share) : gross * share;
    earned += Math.round(credit * 100) / 100;
    counted++;
  });

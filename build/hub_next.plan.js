...
function __plIdentity(){
  ...
  /* Not on the member record. Blanked rather than inherited: an agent seeing
     the broker's licence number on their own profile card is worse than an
     agent seeing a dot. */
  if(!me.license_number) p.licence = '\u00B7';
  p.phone = me.phone || '\u00B7';
  p.area  = '\u00B7';

  /* The frozen rows describe one person's plan and fees. The plan page shows
     these properly, from the file. */
...
/* === Your plan and split ==================================================
   Read only, on purpose. The signed ICA is the system of record for an
   agent's plan, not the Hub, so this page shows the plan and never offers to
   change it.

   No fee is ever computed here. The company fee depends on the plan and on
   whether the property is vacant land, and the rule is reversed between Max
   and Growth. Getting that wrong once put a wrong number in front of an
   agent, so this page only ever shows the fee that was actually recorded on
   the deal. A deal with no fee recorded shows a middle dot.

   Where the recorded fee exceeds the recorded commission the row is flagged
   rather than stated. Ana Puentes' 7896 1st Place carries 3,424 against a
   3,125 commission, which cannot be right if the fee is a share of the
   commission. An agent seeing that unqualified would reasonably think the
   brokerage had taken more than the deal earned.                            */

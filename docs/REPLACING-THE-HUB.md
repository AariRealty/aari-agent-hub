...
## Status check, 7 September 2026

This document was written on 5 September and then sat on
`claude/hub-next-slots`, unmerged, which is why the decision it describes was
not visible to anyone reading `main`. It is landed here on its own. The
branch's `hub_next.html` is deliberately **not** taken: `main`'s copy is three
commits ahead of it, so merging the branch wholesale would have moved the new
build backwards.

**Re-measured against `main` and against what is actually in the bucket today,
not against the branch:**

| Claim in this document | Still true? |
|---|---|
| hub_next carries all three slots | **Yes.** `main` and the served copy both have `TX_SLOT`, `BROKER_SLOT` and `ICA_GATE_SLOT` |
| `realty-hub` returns hub_next early, before the injects | **No longer true.** Both builds now go through one composition path, and the deployed version has that fix |
| hub_next has none of the sidebar contract | **Yes, unchanged.** `.sidebar-item` 0, `data-panel` 0, `.sidebar-group-label` 0, `setPanel` 0, against 59 / 34 / 23 / 1 in hub_payload |
| The bridge has not been written | **Yes, unchanged** |

**So two of the three blockers are gone and the third is the whole job.**

The page shell on Netlify still carries a comment saying hub_next stays a
preview "until TX_SLOT and BROKER_SLOT exist in it and realty-hub stops
returning it early". Both of those conditions have since been met, so that
comment now understates what is left rather than overstating it. What actually
blocks the cutover is the nav bridge in the section above, and nothing else
found so far.

**What this means at the URL.** `hub.joinaari.com` serves hub_payload by
design. `hub.joinaari.com/?hub=next` serves hub_next, broker only, sticky for
the session, and `?hub=live` clears it. Flipping the default is not a flag
change while the bridge is missing: the modules would load and attach to
nothing, and the broker side would go dark silently, which is the failure this

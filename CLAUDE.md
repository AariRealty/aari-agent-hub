...
- The signature move is a **shallow wave** where one band meets the next, 18 to 55px of rise on a 1320px-wide 3x capture, so roughly a 22px curve at
...
- No dashes. Not em dashes, not en dashes, not `\u2014` escapes. A pause is a
  comma or a full stop, a list is introduced with a colon, a range is written
  "9:00 AM to 4:45 PM", and an empty cell in a table is a middle dot. This
  applies to the Hub, to every mockup, and to replies in chat.

// Aari Transactions · tc-invoice-reminder (v9 · weekly STATUS email)
// Pay is the SAME math as /js/pay-engine.js (price table x per-TC pct). Keep in sync if the catalog changes.
//
// v9 · 7 September 2026 · AUTHENTICATION, and the cron that never carried any.
//
// verify_jwt is true here, which the public anon key satisfies, so anyone who had
// read a page's source could fire this. Two things that made worse than noise:
// the default path emails every coordinator and every broker, and a `to` field
// sent the weekly status email to any address the caller named. So it was not
// only "anyone can trigger the batch", it was "anyone can aim it". It now
// requires the service role.
//
// Its cron sent only the anon key as `apikey`, with no Authorization header at
// all, so adding this check without changing the schedule would have silenced a
// live weekly job. The schedule was changed first, to read the service role key
// from vault.decrypted_secrets the way ics-sync-hourly already did, and only then
// was this check added.
...
const SERVICE_PRICE: Record<string, number> = {
  tc_one_side:399, tc_both_sides:599, tc:399, listing_coordinator:179,
...
function jr(status:number, obj:unknown){ return new Response(JSON.stringify(obj), { status, headers:{...CORS,"Content-Type":"application/json"} }); }
...
Deno.serve(async (req) => {
  ...
  if (!isServiceRole(req)) {
    return jr(401, { ok:false, error:"unauthorized", detail:"This emails every coordinator and every broker, and its `to` field can aim a message at any address, so it requires the service role. The public anon key is not it. Its cron job supplies the service role key from vault.decrypted_secrets." });
  }
  ...
  if (body && body.to) {
    ...
    if (!who || !who.length) return jr(400, { ok:false, error:"no agent with that email" });
    const t = who[0];
    if (String(t.role||"")==="broker") {
      ...
      return jr(ok?200:502, { ok, test:true, owner:true, sent_to:t.email, coordinators:team.perTc.length });
    }
    ...
    return jr(ok?200:502, { ok, test:true, sent_to:t.email, files:files.length });
  }
  ...
  return jr(200, { ok:true, coordinators:list.length, sent, owner_reports:ownerSent });
});
